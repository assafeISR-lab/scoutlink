...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const db = await prisma.playerDatabase.findUnique({ where: { id } })
  if (!db) return NextResponse.json({ error: 'Not found' }, { status: 404 })
  if (db.ownerId !== user.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  await prisma.playerDatabase.delete({ where: { id } })
