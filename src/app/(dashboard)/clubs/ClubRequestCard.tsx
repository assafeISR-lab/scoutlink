...
export default function ClubRequestCard({
  ...
  // Build status counts for bottom strip
    ...
    <div className="rounded-2xl border overflow-hidden" style={{ background: 'var(--card-bg)', borderColor: 'var(--border)', boxShadow: 'var(--card-shadow)' }}>
      ...
      <div
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {request.position && (
            <span className="text-sm font-bold" style={{ color: 'var(--text-primary)' }}>{request.position}</span>
          )}
          {!request.position && !request.teamLevel && (
            <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Open request</span>
          )}
          {request.transferType && (
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full"
              style={request.transferType === 'buy'
                ? { background: 'rgba(0,200,150,0.12)', color: '#00c896', border: '1px solid rgba(0,200,150,0.3)' }
                : request.transferType === 'loan'
                ? { background: 'rgba(255,159,67,0.12)', color: '#ff9f43', border: '1px solid rgba(255,159,67,0.3)' }
                : { background: 'rgba(108,143,255,0.12)', color: '#6c8fff', border: '1px solid rgba(108,143,255,0.3)' }}>
              {request.transferType === 'buy' ? 'Buy' : request.transferType === 'loan' ? 'Loan' : 'Free'}
            </span>
          )}
          {(request.ageMin || request.ageMax) && (
            <span className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>
              {request.ageMin && request.ageMax
                ? `${request.ageMin}–${request.ageMax} yrs`
                : request.ageMax
                ? `≤${request.ageMax} yrs`
                : `≥${request.ageMin} yrs`}
            </span>
          ...
          {/* Right-aligned: edit, delete (with confirm inline), chevron */}
          <div className="ml-auto flex items-center gap-1.5 flex-shrink-0" onClick={e => e.stopPropagation()}>
            ...
            <button
              ...
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-faint)'; e.currentTarget.style.borderColor = 'var(--border)' }}>
            ...
            {/* Chevron */}
            <svg className="w-3.5 h-3.5 transition-transform flex-shrink-0" style={{ color: 'var(--text-faint)', transform: expanded ? 'rotate(180deg)' : 'none' }}
              viewBox="0 0 24 24" fill="currentColor"><path d="M7 10l5 5 5-5z"/></svg>
          </div>
        </div>

        {/* Row 2: budget, nationality, notes — only if present */}
        {(request.budget || request.nationality || request.notes) && (
          <div className="flex gap-4 mt-1.5 flex-wrap">
            {request.budget && (
              <span className="text-[11px]" style={{ color: 'var(--text-muted)' }}>
                €{(request.budget / 1000).toFixed(0)}k/yr
              </span>
            )}
            {request.nationality && (
              <span className="text-[11px]" style={{ color: 'var(--text-muted)' }}>{request.nationality}</span>
            )}
            {request.notes && (
              <span className="text-[11px] truncate" style={{ color: 'var(--text-faint)', maxWidth: 200 }}>{request.notes}</span>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-t flex-wrap" style={{ borderColor: 'var(--border)' }}>
        <span className="text-[10px] uppercase font-bold flex-shrink-0" style={{ color: 'var(--text-faint)', letterSpacing: '0.6px' }}>Proposals</span>

        {/* Proposal chips grouped by status */}
        {request.proposals.length === 0 ? (
          ...
          Object.entries(statusCounts).map(([status, count]) => {
            const sc = STATUS_COLORS[status] ?? STATUS_COLORS.proposed
            return (
              <span key={status} className="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full font-medium"
          ...
          {isClosed ? (
            ...
            <>
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #00c896, #00a878)', color: '#fff', boxShadow: '0 2px 8px rgba(0,200,150,0.25)' }}
                onMouseEnter={e => { if (!matching) e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,200,150,0.45)' }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,200,150,0.25)' }}>
                {matching
                  ? <><div className="w-3 h-3 rounded-full border-2 border-current border-t-transparent animate-spin" />Finding…</>
                  : <>Find Players <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z"/></svg></>
                }
              </button>
            </>
          )}
      ...
      {expanded && (
        <div className="p-4 flex flex-col gap-4 border-t" style={{ borderColor: 'var(--border)' }}>
          ...
          {/* Add Manually button + search */}
          {!isClosed && (
            <div className="flex flex-col gap-2">
              <button
                onClick={() => setManualOpen(o => !o)}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold transition-all w-fit"
                style={manualOpen
