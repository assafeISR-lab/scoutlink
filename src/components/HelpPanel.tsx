  ...
  // ── Clubs ────────────────────────────────────────────────────────
  {
    ...
    en: {
      ...
      steps: [
        ...
        'Add clubs using "Add Club". Each club card shows all team levels as clickable pills — click one to filter by that team, or "All" to see everything. Use "⚙ Manage Teams" to add or remove team levels.',
        'The right panel has two views: "Club Requests" (one club at a time) and "All Clubs Requests" (across all clubs).',
        'Each request card shows a team badge, position, transfer type, age range, and proposal chips at a glance. Click "Find Players" (green button on the card) to run an AI match instantly — no need to expand the card first.',
        'Closed requests are collapsed by default. Click "Closed (N)" at the bottom of the requests list to reveal them.',
        'Use the "Filter" button above the requests list to narrow by Position, Transfer type, Nationality, Age, Budget, or proposal status. A blue dot on the button means a filter is active.',
        'In "All Clubs Requests": use the stage tabs at the top (No Proposals / Proposed / In Discussion / Offer / Signed) to quickly see where every open request stands across all clubs.',
      ],
      tip: 'The green "Find Players" button is always visible on each request card — you never need to expand the card to start a match.',
    },
    he: {
      category: 'מועדונים',
      ...
      steps: [
        ...
        'הוסף מועדונים עם "Add Club". כותרת כל מועדון מציגה את רמות הקבוצה כלשוניות לחיצות — לחץ על לשונית לסינון לפי קבוצה, או "All" לראות הכל. השתמש ב-"⚙ Manage Teams" להוספה/הסרה של רמות קבוצה.',
        'לפנל הימני שתי תצוגות: "Club Requests" (מועדון אחד בכל פעם) ו-"All Clubs Requests" (על פני כל המועדונים).',
        'כל כרטיס בקשה מציג תג קבוצה, עמדה, סוג העברה, טווח גיל וצ\'יפים של הצעות במבט ראשון. לחץ "Find Players" (כפתור ירוק בכרטיס) להרצת התאמת AI מיד — ללא צורך לפתוח את הכרטיס תחילה.',
        'בקשות סגורות מכווצות כברירת מחדל. לחץ "Closed (N)" בתחתית רשימת הבקשות לגילויין.',
        'השתמש בכפתור "Filter" מעל רשימת הבקשות לצמצום לפי עמדה, סוג העברה, לאום, גיל, תקציב או סטטוס הצעה. נקודה כחולה על הכפתור מסמנת שפילטר פעיל.',
        'ב-"All Clubs Requests": השתמש בלשוניות השלב בחלק העליון (No Proposals / Proposed / In Discussion / Offer / Signed) לראות במהירות היכן כל בקשה פתוחה עומדת על פני כל המועדונים.',
      ],
      tip: 'כפתור "Find Players" הירוק תמיד גלוי על כל כרטיס בקשה — לעולם אין צורך לפתוח את הכרטיס כדי להתחיל התאמה.',
    },
  },
  {
    id: 'clubs-teams',
